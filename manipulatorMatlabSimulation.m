...
objHeight = 0.2;  % can also be determined later by using object and edge detection in succession.
objRadius = 0.05; % can also be determined later by using object and edge detection in succession.
objPosition = [data(1), data(2), data(3)];
body = rigidBody('objFrame');
setFixedTransform(body.Joint, trvec2tform(objPosition))
...
heightAboveBase = constraintCartesianBounds(gripper);
heightAboveBase.Bounds = [-inf, inf; ...
                           -inf, inf; ...
                           0.05, inf]
distanceFromObj = constraintPositionTarget('objFrame');
distanceFromObj.ReferenceBody = gripper;
distanceFromObj.PositionTolerance = 0.005

alignWithObj = constraintAiming('iiwa_link_ee');
alignWithObj.TargetPoint = [0, 0, 100]
...
distanceFromObj.TargetPosition = [0,0,intermediateDistance];
[qWaypoints(2,:),solutionInfo] = gik(q0, heightAboveBase, ...
                       distanceFromObj, alignWithObj, fixOrientation, ...
                       limitJointChange);
limitJointChange.Weights = ones(size(limitJointChange.Weights));
fixOrientation.Weights = 1;

alignWithObj.Weights = 0;
fixOrientation.TargetOrientation = ...
    tform2quat(getTransform(lbr,qWaypoints(2,:),gripper));

finalDistanceFromObj = 0.05;
distanceFromObjValues = linspace(intermediateDistance, finalDistanceFromObj, numWaypoints-1);
maxJointChange = deg2rad(10);
for k = 3:numWaypoints
    % Update the target position.
    distanceFromObj.TargetPosition(3) = distanceFromObjValues(k-1);
...
hold on
exampleHelperPlotObjAndBase(objHeight, objRadius, objPosition);
